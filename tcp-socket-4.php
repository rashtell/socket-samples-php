...
#region Parsing Line-Delimited Messages, Parsing Commands, Adding Usernames
Loop::run(function () {
    $server = new class
    {
        private $uri = "tcp://127.0.0.1:5024";
        ...
        private function handleMessage(Socket $socket, string $message)
        {
            ...
            if ($message[0] === "/") {
                ...
                $command = strtolower(array_shift($args)); // the first arg is our command name
                ...
                switch ($command) {
                    ...
                    default:
                        $socket->write("Unknown command: {$command}" . PHP_EOL);
                        break;
                }
